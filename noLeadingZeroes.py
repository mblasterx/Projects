...
def compile_formula(formula, verbose=False):
    ...
    body = ''.join(tokens)
    if firstLetters:
        tests = ' and '.join(ltr +'!=0' for ltr in firstLetters)
...
def faster_solve(formula):
    ...
    for digits in itertools.permutations(map(int, set(range(9))), len(letters)):
        try:
            if f(*digits) is True:
...
def test():
    ...
    # assert faster_solve('YOU == ME**2') == ('289 == 17**2' or '576 == 24**2' or '841 == 29**2' or '324 == 18**2'), faster_solve('YOU == ME**2')
    assert faster_solve('X / 0 == X') == None
    return 'tests pass'
